...
A SwiftUI app that consumes the [composemultiplatformsdk](https://github.com/erikg84/composemultiplatformsdk) shared SDK and embeds its 5 Compose Multiplatform screens via **auto-generated** `UIViewControllerRepresentable` wrappers.
...
3. Call the auto-generated `<Screen>Representable()` structs directly inside the navigation

**Zero hand-written wrappers.** The five `*Representable.swift` files under `ComposeMultiplatformImdbDemo/Wrappers/` are generated by the [KMP-ComposeUIViewController](https://github.com/GuilhE/KMP-ComposeUIViewController) KSP plugin from `@ComposeUIViewController`-annotated `@Composable` functions in the SDK's `commonMain`. The SDK release workflow scoops them out of `build/generated/ksp/` and pushes them into this repo via deploy key on every tag. **Do not edit them by hand** — they get overwritten on the next SDK release.
